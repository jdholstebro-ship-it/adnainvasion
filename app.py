...
#
# v4 additions:
#  - DBA / other-name column (from NPPES other_names), so holding-
#    company legal names (e.g. "GOLDEN ROCK HOLDINGS, LLC") reveal
#    the storefront brand that actually matched an org-name search.
#  - Organization-name and Authorized-Official search boxes with
#    wildcard (*) support. Org name is sent to the API; official is
#    filtered client-side (the API doesn't wildcard that field).
# ============================================================
...
def run_search(wildcard, enum_type, state, city, postal_code, org_name, status_area):
    """One query with automatic cap subdivision (unless the user
    already filters by postal code, which conflicts with subdividing).

    `wildcard` may be None — used for name/official searches, where we do
    NOT want to constrain by the Hearing*/Audiolog* taxonomy wildcards
    (that would silently drop target-code records whose description doesn't
    start with those words, e.g. the double-spaced supplier code). Precision
    is still enforced later, client-side, by the target-taxonomy-code filter."""
    base = {"version": "2.1"}
    if wildcard:
        base["taxonomy_description"] = wildcard
    if enum_type:
    ...
    if city:
        base["city"] = city
    if org_name:
        base["organization_name"] = org_name

    label = f"'{wildcard}'" if wildcard else "name search"
    ...
    items, capped = _harvest(base, status_area, label)
...
def to_record(item):
    ...
    other_names = item.get("other_names", []) or []
...
org_name_q = st.sidebar.text_input("Organization name (optional, use * for wildcard)").strip()
official_q = st.sidebar.text_input("Authorized official (optional, use * for wildcard)").strip()
...
st.sidebar.caption(
    ...
    "national coverage, run once per state. Organization-name search only "
    "matches NPI-2 records; put 2+ characters before any *."
)
...
def apply_official_filter(df):
    """Client-side wildcard filter on the authorized official name."""
    rx = wildcard_to_regex(official_q)
    if rx is None or df.empty:
        return df
    return df[df["Authorized_Official"].fillna("").apply(lambda x: bool(rx.search(x)))]
...
if search_clicked:
    ...
    try:
        with st.spinner("Querying NPPES Registry…"):
            # If the user is searching by organization name, search on the
            # NAME alone (no taxonomy wildcard), so target-code records aren't
            # silently dropped by the Hearing*/Audiolog* constraint. The
            # authorized-official search is applied client-side afterward, so
            # it does NOT by itself switch off the taxonomy wildcards — doing
            # so with no other API criterion would make the query invalid.
            # An org name provides the needed API anchor.
            name_mode = bool(org_name_q)
            wildcards = [None] if name_mode else SEARCH_WILDCARDS
            for wc in wildcards:
                items, hit_cap = run_search(
                    wc, ENUM_MAP[enum_choice], state, city, postal_code, org_name_q, status_area
                )
                if hit_cap:
                    capped_queries.append(wc or "name search")
                for it in items:
                    all_items[it.get("number")] = it  # dedupe by NPI

        status_area.empty()
        df = build_results_df(all_items, strict=strict_grouping)
        st.session_state["results_df"] = apply_official_filter(df).reset_index(drop=True)
...
if "results_df" in st.session_state:
    ...
    if df.empty:
        ...
        selected_labels = st.multiselect(
            ...
            default=[f"{v} ({k})" for k, v in CODE_LABELS.items()],
        )
        ...
        st.caption("💡 For verified websites and email addresses, run the downloaded file "
                   "through the local enrichment pipeline (enrich_prospects.py) — it resolves "
                   "each business via Google Places before looking up any emails.")
